...
export interface SWOCommonDecoderOpts {
    type: string; // 'console', 'graph', ...
    port?: number; // ITM Port number
    ports?: number[];
}
...
export interface HostConfig {
    ...
    pvtProxyBindHost?: string; // Resolved bind host for proxy helper launch (e.g. "127.0.0.1" or "0.0.0.0")
    pvtProxyPort?: number; // Proxy port — from Discovery JSON (extension-launched) or configured port (daemon)
    pvtProxyToken?: string; // Token: generated by extension, read from token file, or taken from hostConfig.token
...
export interface ConfigurationArguments extends DebugProtocol.LaunchRequestArguments {
    ...
    // Stuff that should been already defined in DebugProtocol.LaunchRequestArguments
    osx: any;
    linux: any;
    windows: any;
}
...
export function getGDBSWOInitCommands(config: SWOConfiguration): string[] {
    ...
    const commands = [`set $cpuFreq = ${cpuFrequency}`, `set $swoFreq = ${swoFrequency}`, `set $swoPortMask = ${portMask}`, `set $swoFormat = ${swoEncoding === "manchester" ? 1 : 2}`, "SWO_Init"];
...
export function quoteShellAndCmdChars(s: string): string {
...
export class SpawnLineReader extends EventEmitter {
    public callback: null | ((line: string) => boolean) = null;
    private promise: Promise<boolean> | null = null;
    constructor() {
        super();
    }

    public startWithProgram(prog: string, args: readonly string[] = [], spawnOpts: childProcess.SpawnOptions = {}, cb: null | ((line: string) => boolean) = null): Promise<boolean> {
        if (this.promise) {
    ...
    public startWithStream(rStream: stream.Readable, cb: null | ((line: string) => boolean) = null): Promise<boolean> {
        if (this.promise) {
            throw new Error("SpawnLineReader: can't reuse this object");
        }
        this.callback = cb;
    ...
    public startWithFile(filename: fs.PathLike, options?: fs.ReadStreamOptions, cb: null | ((line: string, err?: any) => boolean) = null): Promise<boolean> {
        if (this.promise) {
            throw new Error("SpawnLineReader: can't reuse this object");
        }
        this.callback = cb;
    ...
    private doReadline(rStream: stream.Readable, resolve: (value: boolean) => void) {
        try {
